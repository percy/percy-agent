...
describe('Configuration', () => {
  it('parses valid configuration', () => {
    ...
    expect(subject['static-site']['base-url']).to.eql('/blog')
    expect(subject['static-site']['snapshot-files']).to.eql('**/*.html')
    expect(subject['static-site']['ignore-files']).to.eql('**/*.htm')
  })

  it('gracefully handles a missing file', () => {
    const subject = configuration('test/support/.file-does-not-exist.yml')

    expect(subject).to.eql({'version': 1.0, 'snapshot': {}, 'static-site': {}})
  })
})
